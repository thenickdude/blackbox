...
typedef struct flightLogPrivate_t
{
	...
	//The section of the file which is actually being examined:
	...
	if (log->stats.numIFrames + log->stats.numPFrames <= 1) {
		...
		for (i = 0; i < log->fieldCount; i++) {
			if (log->fieldSigned[i]) {
				...
				log->stats.fieldMaximum[i] = (uint32_t) fields[i] > (uint32_t) log->stats.fieldMaximum[i] ? fields[i] : log->stats.fieldMaximum[i];
				log->stats.fieldMinimum[i] = (uint32_t) fields[i] < (uint32_t) log->stats.fieldMinimum[i] ? fields[i] : log->stats.fieldMinimum[i];
			}
		}
	}
}
...
flightLog_t* parseFlightLog(int fd, FlightLogChooseLog onChooseLog, FlightLogMetadataReady onMetadataReady, FlightLogFrameReady onFrameReady, bool raw)
{
	...
	const char *frameStart = 0, *mapped;

	const char *logSearchStart;
	const char *logBegin[MAX_LOGS_IN_FILE + 1];
	int logIndex, logCount;
    ...
    for (logIndex = 0; logIndex < MAX_LOGS_IN_FILE && logSearchStart < mapped + fileSize; logIndex++) {
		logBegin[logIndex] = memmem(logSearchStart, (mapped + fileSize) - logSearchStart, LOG_START_MARKER, strlen(LOG_START_MARKER));

		if (!logBegin[logIndex])
			break; //No more logs found in the file

		//Search for the next log after this header ends
		logSearchStart = logBegin[logIndex] + strlen(LOG_START_MARKER);
	}
	 ...
	 */
	logBegin[logCount] = mapped + fileSize;

	if (onChooseLog) {
		//Get the caller to choose which log to parse
		logIndex = onChooseLog(logCount, logBegin);
	} else {
		//Default to the first log if caller doesn't care to choose
		logIndex = 0;
	}

	if (logIndex < 0 || logIndex >= logCount) {
		munmap((void*)mapped, fileSize);
	...
	private->logStart = logBegin[logIndex];
    private->logPos = private->logStart;
    private->logEnd = logBegin[logIndex + 1];
				...
				if (lastFrameType == 'P' || lastFrameType == 'I') {
					...
					if (command == 'I' || command == 'P' || command == EOF) {
						...
						if (lastFrameType == 'I') {
							...
							stateIsValid = true;
						} else if (lastFrameType == 'P' && stateIsValid) {
